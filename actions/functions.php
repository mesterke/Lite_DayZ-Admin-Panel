...
//        exit;
	...
	$icon = '<img src="images/icons/'.($row['Alive'] == 0 ? 'dead' : 'live').'.png" title="player '.$player_name.' is '.($row['Alive'] == 0 ? 'dead' : 'live').'" alt=""/>';

        $survival_time =$row['duration'];
        $created = $row['Datestamp'];
        $last_updated  = $row['LastLogin'];
        
	$tablerow = "<tr id='rcon_id_".$player_rcon_id."'>";
           if($show_type == 'online') {

                                $tablerow .="<td align=\"center\" class=\"table-players-td action\"> <img showtime=$show_type onclick=\"ban_player_form('$player_rcon_id','$row[PlayerName]','$player_IP','$player_GUID')\" width=50 title=\"ban player ".$player_name." \" src=\"images/icons/ban.png\"></td>
          ...
          $tablerow .= "<td align=\"center\" class=\"table-players-td\">".$icon."</td>
			...
			<td align=\"center\" class=\"table-players-td\">".$row['PlayerUID']."</td>
...
function row_vehicle($row) {
	...
	for ($i=0; $i< $limit; $i++){
		...
		//$tablerow = "<tr>".$chbox."
			...
			<td align=\"center\" class=\"gear_preview\" ><a target=\"_blank\" href=\"?page=vehicle&id=".$row['ivcid']."\">".$row['Classname']."
                             <br>             
                             <img width=90 src='images/vehicles/".$row['Classname'].".jpg'/> 
                             </a>
                        </td>
	...
	for ($i=0; $i< $limit; $i++){
		if(array_key_exists($i,$Hitpoints)){
			...
			<td align=\"center\" class=\"gear_preview\" >".$row['wvid']."</td>
                        ...
                        <td align=\"center\" class=\"gear_preview\" style=\"color:#000; background-color: rgba(100,".round((255/100)*( ($row['Chance']*100))).",0,0.8);\" ><img width=130 src='images/vehicles/".$row['Classname'].".jpg'/></td>
...
function row_deployable_items($row, $chbox) {
	...
	$tablerow = "<tr>".$chbox."
		...
		<td align=\"center\" class=\"gear_preview\" ><a target=\"_blank\" href=\"?page=player&svid=".$row['svid']."\"> ".$row['PlayerName']."</a></td>
		...
		<td align=\"center\" class=\"gear_preview\" ><a target=\"_blank\" href=\"?page=deployableitem&id=".$row['id']."\">".$row['Classname']."<br> <img width=100 src=\"images/vehicles/".$row['Classname'].".jpg\"> </a></td>			
